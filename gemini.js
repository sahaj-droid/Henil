...
async function _openaiCall(cfg, messages, onChunk) {
  if (!_isValidHttpUrl(cfg.url)) {
    throw new Error('Invalid API URL for ' + cfg.provider);
  }
...
const niviSearchTools = [{
  functionDeclarations: [
    { 
      ...
      description: "Use for general knowledge, world news, or universal facts.", 
      parameters: { 
        type: "OBJECT", 
        properties: { 
          query: { type: "STRING" } 
    ...
    { 
      ...
      description: "Use for Stock Market, Cricket live scores, Coding resources, or Images.", 
      parameters: { 
        type: "OBJECT", 
        properties: { 
          query: { type: "STRING" }, 
          needs_image: { type: "BOOLEAN", description: "True if user wants an image" } 
        }, 
...
window.directGeminiCallStreamMultiTurn = async function(priorHistory, currentPrompt, onChunk) {
  ...
  for (const rawItem of chain) {
    ...
    if (!cfg.model) { 
      lastError = `No model set for ${cfg.provider}`; 
      continue; 
    }
  ...
  if (window.AppState?._abortController?.signal.aborted) {
    return { ok: false, aborted: true };
  }
